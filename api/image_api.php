...
else
{
  ...
  $fileExt = strtolower(pathinfo($img,PATHINFO_EXTENSION)); // get image extension
  ...
  {       
    ...
    if(!file_exists($upload_path . $img))
    {
      // check file size '5MB'
      if($fileSize < 5000000){
        move_uploaded_file($tempPath, $upload_path . $img); // move file from system temporary path to our upload folder path 
      }
      else{   
...
if(!isset($errorMSG))
{
  $query =  mysqli_query($conn,'INSERT into image (img) VALUES("'.$img.'")');
